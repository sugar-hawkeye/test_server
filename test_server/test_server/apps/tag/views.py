...
# from test_server.libs.custom import DataResponse,NotFound
#
#
# class TagList(APIView):
#
...
#             serializer.save()
#             return Response(serializer.data, status=status.HTTP_201_CREATED)
#         return DataResponse(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
#
#
# class TagDetail(APIView):
#
...
#         try:
#             tag = Tag.objects.get(pk=self.request.data['tag_id'])
#         except:
#             # raise NotFound
#             return DataResponse(success=False,message='没有这个对象',data=[])
#         serializer = TagSerializer(tag, data=request.data)
#         if serializer.is_valid():
#             serializer.save()
#             return DataResponse(serializer.data)
...
#         tag.delete()
#         return DataResponse(status=status.HTTP_204_NO_CONTENT)
...
#
#         page = pagination.paginate_queryset(tag,request)
#         if page is not None:
